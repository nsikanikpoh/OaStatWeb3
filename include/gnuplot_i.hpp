...
//Mutex because tmpfile_num is updated in multiple threads.
pthread_mutex_t mutex_tmpfile_num = PTHREAD_MUTEX_INITIALIZER;
...
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__TOS_WIN__)
    if (gnucmd && _pclose(gnucmd) == -1)
#elif defined(unix) || defined(__unix) || defined(__unix__) || defined(__APPLE__)
    if (gnucmd && pclose(gnucmd) == -1)
#endif
...
std::string Gnuplot::create_tmpfile(std::ofstream &tmp)
{
	...
	pthread_mutex_lock( &mutex_tmpfile_num );
    Gnuplot::tmpfile_num++;
	pthread_mutex_unlock( &mutex_tmpfile_num );
